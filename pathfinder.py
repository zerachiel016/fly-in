...
class Pathfinder:
    ...
    def djikstra(hubs, blocked):
        ...
        while queue:
            ...
            for con in [con[0] for con in hubs[cur_zone].connections]:
                ...
                if new_con_cost < table[con][0] and hubs[con].zone != "blocked" and con != blocked:
    ...
    def get_second_path(main_path, hubs):
        ...
        for h in main_path:
            if hubs[h].start or hubs[h].end:
                continue
            path = Pathfinder.djikstra(hubs, h)
            if path[0]:
                paths += [(path[1], path[0])]
